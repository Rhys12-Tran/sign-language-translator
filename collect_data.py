...
collect_data.py
-----------------
Phase 2 of the ASL Recognizer project.

What this script does:
  1. Opens the webcam and mirrors it (so moving your hand right on screen
     matches moving your hand right in real life).
  2. Runs MediaPipe hand detection on every frame and draws the 21
     landmark dots on the hand.
...
# Building this from chr()/ord() instead of typing out "ABCDEFG...Z" by hand
# means there is no risk of a typo dropping or duplicating a letter.
...
# Folder this script lives in, so file paths work no matter which directory
...
def print_collection_summary(sample_counts_by_letter):
    ...
    before moving on to Phase 3 (training the classifier).
    """
...
def main():
    ...
    # The MediaPipe Tasks API needs these specific classes to configure and
    # build a hand detector.
